...
class MapsAPI:
    ...
    async def get_shapes(self, shape: str, *, params: Mapping[str, Any] | None = None) -> MapShapeResponse:
        query = {"shape": shape}
        if params:
    ...
    async def get_series_group(self, series_id: str, *, params: Mapping[str, Any] | None = None) -> MapSeriesGroupResponse:
    ...
    async def get_regional_data(self, *, params: Mapping[str, Any] | None = None) -> MapRegionalDataResponse:
        data = await self._client.get("/geofred/regional/data", params=params)
        return MapRegionalDataResponse.model_validate(data)

    async def get_series_data(self, series_id: str, *, params: Mapping[str, Any] | None = None) -> MapSeriesDataResponse:
        query = {"series_id": series_id}
        if params:
            query.update(dict(params))
